"""Desktop GUI for Subtitle Tool — Batch processing with queue."""
...
class SubtitleApp(ctk.CTk):
    """Main application window with batch processing."""

    def __init__(self) -> None:
        super().__init__()

        self.title("🎬 Daisy Subtitle Tool")
    ...
    def _build_ui(self) -> None:
        ...
        ctk.CTkLabel(
            title_row, text="Batch", font=ctk.CTkFont(size=11),
            text_color="gray",
        ...
        # FFmpeg
        ff_col = ctk.CTkFrame(settings, fg_color="transparent")
        ...
        # Model
        m_col = ctk.CTkFrame(settings, fg_color="transparent")
        ...
        ctk.CTkOptionMenu(
            m_col, values=["tiny", "base", "small", "medium"],
            variable=self.model_var, width=100, height=32,
        ).pack(pady=(3, 0))

        # Watermark
        w_col = ctk.CTkFrame(settings, fg_color="transparent")
        w_col.pack(side="left", padx=(0, 10))
        ctk.CTkLabel(w_col, text="Watermark", font=ctk.CTkFont(size=12)).pack(anchor="w")
        self.wm_entry = ctk.CTkEntry(w_col, width=100, height=32, font=ctk.CTkFont(size=12))
        self.wm_entry.insert(0, "Daisy")
        self.wm_entry.pack(pady=(3, 0))

        # Output
        o_col = ctk.CTkFrame(settings, fg_color="transparent")
        o_col.pack(side="left")
        ctk.CTkLabel(o_col, text="Output", font=ctk.CTkFont(size=12)).pack(anchor="w")
        o_row = ctk.CTkFrame(o_col, fg_color="transparent")
        o_row.pack(pady=(3, 0))
        self.out_entry = ctk.CTkEntry(o_row, width=120, height=32, font=ctk.CTkFont(size=12))
        self.out_entry.insert(0, "./output")
        self.out_entry.pack(side="left", padx=(0, 4))
        ctk.CTkButton(
    ...
    def _on_start(self) -> None:
        ...
        if not ffmpeg or not Path(ffmpeg).exists():
            self._set_status("⚠ Select a valid ffmpeg.exe", "#e74c3c")
            return
    ...
    def _process_one(self, item: VideoItem, prefix: str) -> None:
        """Process a single video using the shared pipeline with progress."""
        from subtitle_tool.pipeline import process_single_video

        ffmpeg = self.ffmpeg_entry.get().strip()
        config = AppConfig(
            output_dir=Path(self.out_entry.get().strip() or "./output"),
            ffmpeg_path=ffmpeg,
        )
        config.whisper.model_size = self.model_var.get()
        config.watermark.text = self.wm_entry.get().strip() or "Daisy"

        self.after(0, item.set_status, "downloading")
        self._set_status(f"{prefix} Downloading...")
        ...
        def on_progress(current_sec: float, total_sec: float, speed: str) -> None:
            ...
            self.after(0, item.set_status, "encoding", f"{pct*100:.0f}%  {speed}")

        result = process_single_video(item.url, config, progress_callback=on_progress)
        ...
        if result.success and result.output_path:
            ...
            # Show quality score alongside file size
            quality_info = ""
            if result.quality_score is not None:
                grade_map = {90: "A", 80: "B", 70: "C", 60: "D"}
                grade = "F"
                for threshold, letter in sorted(grade_map.items(), reverse=True):
                    if result.quality_score >= threshold:
                        grade = letter
                        break
                quality_info = f" · Q:{result.quality_score:.0f}({grade})"
            self.after(0, item.set_status, "done", f"{size_mb:.0f} MB{quality_info}")
